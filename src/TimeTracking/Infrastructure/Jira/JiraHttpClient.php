...
final readonly class JiraHttpClient implements JiraTransport
{
    ...
    private function errorMessage(int $status, string $content): string
    {
        ...
        if (is_array($details)) {
            ...
            foreach (is_array($details['errors'] ?? null) ? $details['errors'] : [] as $field => $message) {
                if (is_string($message) && trim($message) !== '') {
                    $prefix = is_string($field) && $field !== '' ? "{$field}: " : '';
                    $messages[] = $prefix . trim($message);
                }
            }
        }
