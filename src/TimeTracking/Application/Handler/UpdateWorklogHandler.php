...
final readonly class UpdateWorklogHandler
{
    ...
    public function handle(UpdateWorklog $command): SavedWorklog
    {
        try {
            $issue = IssueKey::fromString($command->issue);
            $date = WorkDate::fromString($command->date);
            $this->gateway->update(new Worklog(
                $issue,
                WorklogId::fromString($command->id),
